...
namespace putki
{
	...
	void write_java_runtime_class(putki::parsed_file *file, putki::indentedwriter out, putki::indentedwriter switch_case_out, putki::indentedwriter switch_case_resolve)
	{
		/*
		...
		for (size_t i=0; i!=file->structs.size(); i++)
		{
			...
			for (size_t i=0; i<s->fields.size(); i++)
			{
				...
				out.line() << "public " << java_type_name(&s->fields[i]);
			...
			for (size_t i=0; i<s->fields.size(); i++)
			{
				...
				if (s->fields[i].is_array)
				{
					...
					out.line() << "aux.Skip(count * " << java_type_size_expr(&s->fields[i]) << ");";
					out.line() << field_ref << " = new " << java_type_name(&s->fields[i]) << "[count];";
			...
			if (!s->parent.empty())
				out.line() << "super.ResolveFromPackage(pkg);";
